...
def run_experiment(probs_heads, n_tosses, n_iters = 100, seed = 389, correct_the_pvalues = False):
    '''
    Input: prob_heads, a list of floats, the probability of a simulated coin toss returning heads
           n_toss, a list of integers, the number of coin tosses to simulate
    ...
    # loop through the list of probability heads
    for i, prob_heads in enumerate(probs):
        # loop through the number of tosses
        for j, n_toss in enumerate(tosses):
            ...
            # IF corrected pvalues are wanted
...
# create the matrix of uncorrected power values
power_mat_uncorr = run_experiment(probs, tosses)
...
# create the matrix of corrected power values
power_mat_corr = run_experiment(probs, tosses, correct_the_pvalues=True)

# plot the corrected heatmap of power values
fig, ax = plt.subplots()
ax = sns.heatmap(power_mat_corr,
                vmin=0,
                vmax=1,
                cmap="Blues",
                xticklabels=tosses,
