...
# make a graph, specifically a scatterplot

genesoi = ["PIM1", "SMYD3", "FADS1", "PRKAR2B", "GATA1", "MYC"]
genesoilocs = []

for genoi in genesoi:
    genesoilocs.append(np.where(np.array(gene_names) == genoi)[0][0])
...
for geneoi, idx in zip(genesoi,genesoilocs): # zip enumerate through both lists and grab genes and index
    ax.text(k562_model_predictions[idx], k562_observations[idx], geneoi, color="maroon", fontweight="demi")
ax.spines.right.set_visible(False)
ax.spines.top.set_visible(False)
plt.tight_layout()
plt.savefig("recreate_fig3.png")
plt.close
